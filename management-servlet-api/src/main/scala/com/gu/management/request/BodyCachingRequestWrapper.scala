...
class BodyCachingRequestWrapper(val request: HttpServletRequest) extends HttpServletRequestWrapper(request)
  ...
  lazy val params: Map[String, List[String]] = addToMultiMap(request.parameters, formParams)
  ...
  override def getParameterValues(name: String): Array[String] = params.get(name) map { _.toArray } getOrElse null
  override def getParameterMap: java.util.Map[String, Array[String]] = params mapValues { _.toArray }
